        ...
        if (message.role === "assistant" && Array.isArray(content)) {
            ...
            if (toolUseIds.length > 0) {
                ...
                const resultBlocks = toolUseIds.map((toolUseId) => {
                    const matchedIndex = nextContent?.findIndex((block, blockIndex) => {
                        if (usedResultIndexes.has(blockIndex))
                            return false;
...
function omittedToolResult(toolUseId) {
    return {
        type: "tool_result",
...
const TRAILING_USER_PROMPT = "The previous turn produced no tool result. If you tried to call a tool, it was not recognized as a valid tool call (likely a formatting problem, or extra text emitted after the call). Re-issue the tool call on its own using the correct format. Otherwise, continue.";
/**
 ...
 * continues.
 */
export function ensureTrailingUser(messages) {

...
export const BILLING_SYSTEM_MARKER = encoder.encode(
...
type BunHashGlobal = typeof globalThis & {
  Bun?: {
    hash?: {
      xxHash64?: (data: Uint8Array, seed: bigint) => bigint
    }
  }
}
...
function round64(accumulator: bigint, input: bigint): bigint {
  let acc = u64(accumulator + u64(input * PRIME64_2))
  acc = rotl64(acc, 31)
  return u64(acc * PRIME64_1)
}

function mergeRound64(accumulator: bigint, value: bigint): bigint {
...
export function fallbackXXH64(data: Uint8Array, seed: bigint): bigint {
...
export function xxHash64(data: Uint8Array, seed: bigint): bigint {
  ...
  if (bunXXH64) return bunXXH64(data, seed)
  return fallbackXXH64(data, seed)
}

export function patchCch(
  body: Uint8Array,
...
export function wrapFetchForCch(base: typeof fetch): typeof fetch {
  return ((input: RequestInfo | URL, init?: RequestInit) => {
    if (
      ...
      if (patchCch(encoded) === "unanchored") {
        console.warn(
          "opencode-claude-auth: cch billing placeholder present but not patched; sending unattested request",
        )
      }
